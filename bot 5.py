...
def a330_kb(chat_id: int) -> InlineKeyboardMarkup:
    a330 = get_a330(chat_id)
    rows = []
    for i, block in enumerate(A330_BLOCKS):
        status = a330.get(block, 0)
        icon = ["⬜", "🟡", "✅"][status]
        rows.append([InlineKeyboardButton(text=f"{icon} {block}", callback_data=f"a330:{i}")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def cb_a330(callback: CallbackQuery):
    ...
    block = BLOCK_CODE.get(code)
    if block is None:
        await callback.answer("Не удалось определить блок", show_alert=True)
        return
